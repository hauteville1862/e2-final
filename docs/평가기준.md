...
(1) **파일을 여닫는 코드는 [`storage.py`](../storage.py)에만 있다.** `quiz_cli.py`, `main.py`에는 `import json`도 `open()` 호출도 없다. 경로·인코딩·손상 복구는 [`Storage.load`](../storage.py#L41-L70)/[`Storage.save`](../storage.py#L72-L81) 안에 숨겨져 있고, 바깥은 이 두 메서드만 부른다.

(2) **`Storage` 객체는 한 번만 생성돼 재사용된다.** `QuizCLI` 생성 시([`quiz_cli.py:8`](../quiz_cli.py#L8)) 만들어진 뒤 계속 쓰인다.

(3) **하지만 데이터는 캐싱하지 않는다.** 메뉴 동작이 시작될 때마다 `load()`를 다시 호출해 파일을 처음부터 새로 읽는다.

(4) **쓰기는 데이터를 바꾼 경우에만** 동작 끝에서 `save()`를 호출해 파일 전체를 다시 쓴다. 즉 메뉴를 선택할 때마다 파일 전체를 다시 읽고, 바뀐 경우만 전체를 다시 쓰는 구조다.

| 메서드 | 파일 | load 위치 | save 위치 | 비고 |
|---|---|---|---|---|
| [`run_quiz`](../quiz_cli.py#L80-L111) | `quiz_cli.py` | L81 (시작) | `save_result` 내부 L77 (정상 종료 L108 / 중단 L99에서 호출) | 진행 중엔 메모리(`data`, `game`)만 사용 |
| [`add_new_quiz`](../quiz_cli.py#L113-L134) | `quiz_cli.py` | L124 | L129 | load → append → save 순 |
| [`view_quiz_list`](../quiz_cli.py#L136-L147) | `quiz_cli.py` | L137 | 없음 | 읽기 전용 |
| [`show_best_score`](../quiz_cli.py#L149-L162) | `quiz_cli.py` | L150 | 없음 | 읽기 전용 |

```mermaid
flowchart TD
    Init(["QuizCLI 생성 시<br/>Storage 객체 1회 생성 (2)"]) --> Menu["메인 메뉴 반복"]

    Menu -->|1. 퀴즈 풀기| RQ["run_quiz<br/>load() L81 (3)"]
    Menu -->|2. 퀴즈 추가| AQ["add_new_quiz<br/>load() L124 (3)"]
    Menu -->|3. 목록 보기| VL["view_quiz_list<br/>load() L137 (3)"]
    Menu -->|4. 점수 확인| SB["show_best_score<br/>load() L150 (3)"]

    RQ --> RQW["메모리에서 게임 진행"]
    RQW --> RQS["save_result → save() L77 (4)"]
