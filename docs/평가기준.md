...
(2) 선택지 개수가 바뀐다면
- [`Quiz.show`](../quiz.py#L7-L10): `choices` 리스트를 `for`문으로 순회해 출력하므로 개수와 무관하게 그대로 동작한다. 고칠 필요 없음.
- [`add_new_quiz`](../quiz_cli.py#L113-L134): 보기 입력 개수(`range(1, 5)`)와 정답 번호 검증 범위(`1, 4`)가 4로 고정되어 있는 곳.
- [`ask_one_quiz`](../quiz_cli.py#L54-L67): 퀴즈를 **풀 때** 정답 입력을 받는 `get_valid_int("...", 1, 4)`도 4로 고정되어 있음. 이 부분을 놓치면 `add_new_quiz`에서 5번·6번 선택지를 등록해도 실제 플레이 중에는 5·6번을 입력할 수 없어 그 문제를 맞힐 방법이 없어지는 버그가 생긴다.
- 즉 선택지 개수를 바꾸려면 위 두 메서드에 흩어진 "4"를 모두 찾아 함께 고쳐야 한다.
