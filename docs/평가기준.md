...
**1. `git checkout`으로 새 브랜치를 만들어 작업함.
...
2. 작업을 마친 뒤 `git checkout`으로 돌아와 `git merge`로 병합.
...
**1. `git clone`으로 저장소를 다른 환경(macOS)에 복제.**
- `cd ~/e2-final`: 복제된 폴더로 이동
- `code .`: 현재 폴더(`.`)를 VS Code 작업 폴더로 열기.
...
**2. 복제본에서 README를 고쳐 commit 후 push하고, 원래 작업 디렉터리에서 `git pull`로 가져옴.**
...
(3) `load()`를 호출하는 시점은 메서드마다 다름.

(4) 데이터를 바꾼 경우에만 동작 끝에서 `save()`를 호출해 파일 전체를 다시 씀.

- [`run_quiz`](../quiz_cli.py#L80-L111): 첫 줄에서 load, 정상 종료·중단 시 저장 (진행 중엔 메모리만 사용)
- [`add_new_quiz`](../quiz_cli.py#L113-L134): 입력을 모두 받은 뒤 load → append → save 순
...
flowchart TD
    ...
    Menu -->|2. 퀴즈 추가| AQI["add_new_quiz<br/>입력 받기 (문제·보기4개·정답)"]
...
try/except를 두 군데로 나눠 이중으로 처리함.

(1) 안쪽 — [`QuizCLI.run_quiz`](../quiz_cli.py#L91-L104)

(2) 바깥쪽 — [`main.py`](../main.py#L6-L15): `sys.exit(0)`으로 정상 종료 코드를 반환.
...
(1) `best_score`, `play_count`를 `quizzes` (최상위 필드): 점수 확인 메뉴에서 `quizzes` 리스트를 순회할 필요 없이 바로 값을 읽을 수 있음.

(2) `quizzes`: 딕셔너리 리스트로, 각 원소가 `question`/`choices`/`answer` 세 키만 가짐. 딕셔너리 구조가 `Quiz` 생성자 인자와 1:1로 대응됨.
...
(1) 점수 계산 방식이 바뀐다면
- [`QuizGame.submit_answer`](../quiz_game.py#L13-L20): 한 문제를 채점해 점수에 반영하는 곳.
- [`save_result`](../quiz_cli.py#L69-L78): 한 판이 끝난 뒤 최고 점수와 비교·갱신하는 곳.

(2) 선택지 개수가 바뀐다면
- [`add_new_quiz`](../quiz_cli.py#L113-L134): 보기 입력 개수(`range(1, 5)`)와 정답 번호 검증 범위(`1, 4`)가 4로 고정되어 있는 곳.
- [`ask_one_quiz`](../quiz_cli.py#L54-L67): 퀴즈를 풀 때 정답 입력을 받는 `get_valid_int("...", 1, 4)` 또한 4로 고정되어 있는 곳.
