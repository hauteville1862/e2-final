...
- Python 기본 문법을 사용해 입력/출력 흐름을 만듭니다.
- 클래스(객체 지향)로 코드를 역할별로 구조화합니다.
...
1. 미션 수행 저장소를 clone하여 별도의 로컬 디렉터리에 복제한다.
2. 복제된 저장소에서 간단한 변경(예: README에 한 줄 추가)을 하고 commit → push한다.
3. 기존 로컬 작업 디렉터리에서 pull로 변경사항을 가져온다.
